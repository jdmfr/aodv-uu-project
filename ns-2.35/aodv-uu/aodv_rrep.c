...
HELLO* NS_CLASS hello_create(struct in_addr origin_addr, u_int32_t dest_seqno, u_int8_t channel){
	HELLO* hello;

	hello = (HELLO*)malloc(sizeof(HELLO));
	hello->type = AODV_HELLO;
	hello->origin_addr = origin_addr.s_addr;
...
HELLO_ACK* NS_CLASS hello_ack_create(struct in_addr dest_addr, struct in_addr origin_addr, u_int8_t channel, u_int8_t stability){
	HELLO_ACK* hello_ack;

	hello_ack = (HELLO_ACK*)malloc(sizeof(HELLO_ACK));
	hello_ack->type = AODV_HELLO_ACK;
	hello_ack->dest_addr = dest_addr.s_addr;
	hello_ack->origin_addr = origin_addr.s_addr;
	hello_ack->channel = channel;
	hello_ack->stability = stability;
...
#endif
//16231053+++++++++++++++++++++++++++++++++++++
    u_int32_t rrep_Channel,rrep_Cost;
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//16231053++++++++++++++++++++++++++++++++++++++++++++++++++
    rrep_Cost = rrep->Cost + Func_La(ip_src,DEV_IFINDEX(ifindex).ipaddr);
    rrep_Channel = Func_Cha(ip_src,DEV_IFINDEX(ifindex).ipaddr);
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//16231053+++++++++++++++++++++++++++++++++++++++++++
    ...
    fprintf(stderr, "16231053:rrep_process===============================================\n");
    fprintf(stderr, "L->I %d-->%d  from %d-->%d Channel=%d Cost=%d\n",ip_src,DEV_IFINDEX(ifindex).ipaddr, rrep_orig, rrep_dest,rrep_Channel,rrep_Cost);
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++=
    if (!fwd_rt) {
	/* We didn't have an existing entry, so we insert a new one. */
	fwd_rt = rt_table_insert(rrep_dest, ip_src, rrep_new_hcnt, rrep_seqno,
				 rrep_lifetime, VALID, rt_flags, ifindex,rrep_Cost,rrep_Channel);//16231053++++++++++++++++++++++++++++++++++
	...
	if (inet_rrep) {
		...
		rt_table_insert(inet_dest_addr, rrep_dest, rrep_new_hcnt, 0,
				rrep_lifetime, VALID, RT_INET_DEST, ifindex,rrep_Cost,rrep_Channel);//16231053+++++++++++++++++++++++++++++
		...
		rt_table_update(inet_rt, rrep_dest, rrep_new_hcnt, 0,
				...
				inet_rt->flags,rrep_Cost,rrep_Channel);//16231053+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	    } else {
