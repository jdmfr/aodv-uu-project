                           ...
                           unsigned int ifindex)
{
    ...
    rrdp->Cost =rrdp_Cost = rrdp->Cost + Func_La(ip_src,DEV_IFINDEX(ifindex).ipaddr);
    rrdp_Channel = Func_Cha(ip_src,DEV_IFINDEX(ifindex).ipaddr);
    fprintf(stderr,"process rrdp ,from :%d to: %d ,Cost : %d ,Channel: %d\n",ip_to_str(ip_src),ip_to_str(ip_dst),rrdp->Cost,rrdp->Channel);
    ...
    rev_rt = rt_table_find(rrdp_orig);
     ...
     if (!fwd_rt) {
        ...
        fwd_rt = rt_table_update(fwd_rt, ip_src, rrdp_new_hcnt, rrdp_seqno,
         ...
         fprintf(stderr,"rrdp update the route table\n");
    } else {
