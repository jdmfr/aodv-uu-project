                        ...
                        int ttl, u_int8_t flags)
{
    ...
    for (i = 0; i < MAX_NR_INTERFACES; i++) {
        ...
        fprintf(stderr,"going to send rrdq\n");
                           ...
                           unsigned int ifindex)
{
    ...
    rrdq->Cost = rrdq->Cost + Func_La(ip_src,DEV_IFINDEX(ifindex).ipaddr);
    rrdq_Channel = Func_Cha(ip_src,DEV_IFINDEX(ifindex).ipaddr);

    fprintf(stderr,  " ip_src=%s rrdq_orig=%s rrdq_dest=%s ttl=%d , cuurent=%d ,Cost : %d ,Channel: %d\n",
            ip_to_str(ip_src), ip_to_str(rrdq_orig), ip_to_str(rrdq_dest),
            ip_ttl,DEV_IFINDEX(ifindex).ipaddr.s_addr,rrdq->Cost,rrdq->Channel);
 ...
 //       ;
    ...
    if (rrdq_dest.s_addr == DEV_IFINDEX(ifindex).ipaddr.s_addr) {
        ...
        rrdp=rrdp_create(0, rev_rt->dest_addr ,this_host.seqno, \
                        DEV_IFINDEX(rev_rt->ifindex).ipaddr, MY_ROUTE_TIMEOUT,rrdq_Channel);
