			   ...
			   unsigned int ifindex)
{
   ...
   // u_int32_t rreq_Cost,rreq_Channel;
	//rreq_Cost = Func_La(source,DEV_IFINDEX(ifindex).ipaddr);
   // rreq_Channel = Func_Cha(source,DEV_IFINDEX(ifindex).ipaddr);
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	rt = rt_table_insert(source, source, 1, 0,
			     ACTIVE_ROUTE_TIMEOUT, VALID, 0, ifindex,-1,-1);//16231053++++++++++++++++++++++++++++++++++++++
    } else {
...
void NS_CLASS neighbor_link_break(rt_table_t * rt)
{
	...
	list_foreach(pos, &rt_tbl.tbl[i]) {
		...
		if (rt_u->nprec) {
		    ...
		    if (!rerr) {
			rerr =
			    rerr_create(0, rt_u->dest_addr, rt_u->dest_seqno,0);
