...
//16231053+++++++++++++++++++++++++++++++++++++++++++
    rreq->Cost = 0;
    rreq->Channel = 0;
//initialize rreq Cost and Channel+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    rreq_Cost = rreq->Cost + Func_La(ip_src,DEV_IFINDEX(ifindex).ipaddr);
    rreq_Channel = Func_Cha(ip_src,DEV_IFINDEX(ifindex).ipaddr);
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if (rreq_record_find(rreq_orig, rreq_id,ip_src))//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        return;

    /* Now buffer this RREQ so that we don't process a similar RREQ we
       get within PATH_DISCOVERY_TIME. */
    rreq_record_insert(rreq_orig, rreq_id,ip_src);
//16231053+++++++++++++++++++++++++++++++++++++++++++++++++++
    fprintf(stderr, "16231053:rreq_process++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
    fprintf(stderr, "16231053:L->I %d-->%d from %d-->%d  Cost=%d la=%d \n",ip_src,DEV_IFINDEX(ifindex).ipaddr,rreq_orig,rreq_dest,rreq_Cost,Func_La(ip_src,DEV_IFINDEX(ifindex).ipaddr));
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if (rev_rt == NULL) {
        ...
        rev_rt = rt_table_insert(rreq_orig, ip_src, rreq_new_hcnt,
                                 rreq_orig_seqno, life, VALID, 0, ifindex,rreq_Cost,rreq_Channel);//16231053++++++++++++++++++++++++++++++++
//16231053+++++++++++++++++++++
fprintf(stderr, "insert into the rt_table with cost=%d channel=%d\n",rreq_Cost,rreq_Channel);
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    } else {
        if (rev_rt->dest_seqno == 0 ||
                (int32_t) rreq_orig_seqno > (int32_t) rev_rt->dest_seqno ||
                (rreq_orig_seqno == rev_rt->dest_seqno &&
                 (rev_rt->state == INVALID || rreq_Cost < rev_rt->Cost))) {//16231053+++++++++++++++++++++++++++++++++++++++++++++++++++++++
//16231053+++++++++++++++++++++
fprintf(stderr, "update the rt_table with cost=%d channel=%d\n",rreq_Cost,rreq_Channel);
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            rev_rt = rt_table_update(rev_rt, ip_src, rreq_new_hcnt,
                                     rreq_orig_seqno, life, VALID,
                                     rev_rt->flags,rreq_Cost,rreq_Channel);//16231053+++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if (internet_gw_mode) {
        ...
        case HOST_INET:
            ...
            rrep = rrep_create(0, 0, 0, DEV_IFINDEX(rev_rt->ifindex).ipaddr,
                               ...
                               ACTIVE_ROUTE_TIMEOUT,rev_rt->Channel);//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if (rreq_dest.s_addr == DEV_IFINDEX(ifindex).ipaddr.s_addr) {
        ...
        /* WE are the RREQ DESTINATION. Update the node's own
           ...
           one in the RREQ. */
//16231053+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	fprintf(stderr, "16231053:i am the destination =%d\n",DEV_IFINDEX(ifindex).ipaddr);
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        rrep = rrep_create(0, 0, 0, DEV_IFINDEX(rev_rt->ifindex).ipaddr,
                           this_host.seqno, rev_rt->dest_addr,
                           MY_ROUTE_TIMEOUT,rev_rt->Channel);//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//16231053+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	fprintf(stderr, "16231053:create a rrep channel=%d next=%d \n",rev_rt->Channel,rev_rt->next_hop);
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        rrep_send(rrep, rev_rt, NULL, RREP_SIZE);
        ...
        if (fwd_rt && fwd_rt->state == VALID && !rreq->d) {
            ...
            if (fwd_rt->flags & RT_INET_DEST) {
                ...
                rrep = rrep_create(0, 0, gw_rt->hcnt, gw_rt->dest_addr,
                                   ...
                                   lifetime,rev_rt->Channel);//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
NS_STATIC struct rreq_record *NS_CLASS rreq_record_insert(struct in_addr
        orig_addr,
        u_int32_t rreq_id,struct in_addr src_addr)//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
{
    struct rreq_record *rec;

    /* First check if this rreq packet is already buffered */
    rec = rreq_record_find(orig_addr, rreq_id,src_addr);//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    rec->rreq_id = rreq_id;
    rec->src_addr = src_addr;//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        u_int32_t rreq_id,struct in_addr src_addr)//16231053+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
{
    ...
    list_foreach(pos, &rreq_records) {
        ...
        if (rec->orig_addr.s_addr == orig_addr.s_addr &&
                (rec->rreq_id == rreq_id) &&(rec->src_addr.s_addr == src_addr.s_addr))//16231053+++++++++++++++++++++++++++++++++++++++++++++++
            return rec;
    }
    return NULL;
}
