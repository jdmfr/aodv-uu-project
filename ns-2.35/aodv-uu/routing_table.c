...
//16231053++++++++++++++++++++++++++++++++++++++++++++++++
u_int32_t NS_CLASS Func_La(struct in_addr src,struct in_addr dest){
	u_int32_t x,y;
	u_int32_t La[5][5]={{0,3,2,1,10},{3,0,2,10,3},{2,2,0,2,2},{1,10,2,0,1},{10,3,2,1,0}};
	x=src.s_addr;
	y=dest.s_addr;
	return La[x][y];
}

u_int32_t NS_CLASS Func_Cha(struct in_addr src,struct in_addr dest){
	u_int32_t x,y;
	u_int32_t Channel[5][5]={{0,1,2,0,0},{1,0,1,0,2},{2,1,0,1,0},{0,0,1,0,0},{0,2,0,0,0}};
	x=src.s_addr;
	y=dest.s_addr;	
	return Channel[x][y];
}
...
//updated 2018/12/21 by buaa16061132
int NS_CLASS ARIMA_prediction(int time_chain[5]){
	std::vector<double> dataArray;
	dataArray.push_back(time_chain[0]);
	dataArray.push_back(time_chain[1]);
	dataArray.push_back(time_chain[2]);
	dataArray.push_back(time_chain[3]);
	dataArray.push_back(time_chain[4]);

	if(time_chain[4] == 1000){
		dataArray.push_back(999);
		dataArray.push_back(1000);
		dataArray.push_back(1000);
	}
	else if(time_chain[4] == 0){
...
rt_table_t *NS_CLASS rt_table_insert(struct in_addr dest_addr,
				     ...
				     u_int16_t flags, unsigned int ifindex,u_int32_t Cost,u_int32_t Channel)//16231053+++++++++++++++++++++
{
	hash_value hash;
	unsigned int index;
	list_t *pos;
...
rt_table_t *NS_CLASS rt_table_update(rt_table_t * rt, struct in_addr next,
				     ...
				     u_int16_t flags,u_int32_t Cost,u_int32_t Channel)//16231053+++++++++++++++++++++++++++++++++++++++++++
{
	struct in_addr nm;
	nm.s_addr = 0;
		...
		list_foreach(pos, &rt_tbl.tbl[i]) {
			...
			if (rt->flags & RT_INET_DEST && rt->state == VALID) {
				rt_table_update(rt, gw->dest_addr, gw->hcnt, 0,
						life, VALID, rt->flags,-1,-1);//16231053++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				n++;
			}
		}
	}
