...
/*    if ((aodv_msg->type == AODV_RREP && ttl == 1 &&
    ...
    */
    ...
    if ((aodv_msg->type == AODV_HELLO && ttl == 1 &&
         dst.s_addr == AODV_BROADCAST)) {
        hello_process((HELLO*) aodv_msg, len, ifindex);
        return;
    }
    else if(aodv_msg->type == AODV_HELLO_ACK){
        hello_ack_process((HELLO_ACK*)aodv_msg, len, ifindex);  //may introduce the hello ack process function
        return;
    }
    else if(aodv_msg->type==AODV_HELLO){
        fprintf(stderr,"this is a hello !!!!!!! by dormouse \n");
